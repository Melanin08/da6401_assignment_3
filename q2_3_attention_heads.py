...
def attention_statistics(attn: torch.Tensor) -> dict[str, float]:
    ...
    next_token = (
        torch.diagonal(attn[:, 1:], offset=0).mean().item()
    ...
    expected_distance = (
        (attn * distance).sum(dim=-1).mean().item()
    )

    entropy = -(
        attn.clamp_min(1e-12)
        * attn.clamp_min(1e-12).log()
    ).sum(dim=-1).mean().item()
...
def log_last_encoder_attention(
    ...
    # The MultiHeadAttention module stores weights
    # from its latest forward pass.
    attn = model.encoder.layers[-1].self_attn.attn_weights

    if attn is None:
        raise RuntimeError(
            "No attention weights found on the last encoder layer"
        )
    ...
    for head_idx in range(attn.size(0)):
        ...
        wandb.log({
            ...
            "next_token_attention": stats["next_token_attention"],
            "previous_token_attention": stats["previous_token_attention"],
            "expected_attention_distance": stats["expected_attention_distance"],
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Log last encoder layer attention heads to W&B"
    )
    ...
    parser.add_argument(
        "--sentence",
        default=None,
        help="German source sentence to visualize"
    )
    ...
    if not Path(args.checkpoint).exists():
        raise FileNotFoundError(
            f"{args.checkpoint} was not found. "
            f"Train the main model first with: python train.py"
        )
    ...
    sentence, labels, src = encode_source_sentence(
        dataset,
        args.sentence
    )
    ...
    wandb.log({
        "visualized_sentence": sentence
    })

    log_last_encoder_attention(
        model,
        src,
        labels,
        dataset.src_vocab.pad_idx,
        device
    )
